...
| Frost | 氷雪 | 雪 | `a.frostbite`1:凍傷(相手の行動順番-1して遅らせる) | `a.ice-reflect`1:氷属性のダメージ反射(3/10) | `e.ice+20`, `r.fire_x1.3`, `r.ice_x1/5` |
...
| Titan | 巨人 | 巨 | `a.colossal`1:巨人(防御力が2倍になり、物理ダメージ補正x2.0(値が大きいほど大ダメージ)| `a.mutual-physical-amplify`-1:物理抑制(双方物理ダメージ0.8倍) | `c.growth_x1.5` |
...
- Distributions: A(Most common) B(Secondary, later part), C(Accent for floor 3, limited), D(Boss and symbolic)
...
<EnemyTypeSource>
- `A`: Most common enemy type
- `B`: Secondary enemy type
- `C`: Accent enemy type
- `D`: Boss / symbolic enemy type