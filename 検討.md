...
- **Gods Battle (神魔戦)**
  - Loot Gate Condition: Collect 20 Boss rare items in dungeons to unlock Gods Battle.
  - UI / Trigger:
...
ExHP(1)=1, ExHP(n)= ExHP(n-1)*(4 - 0.3*(ex -2))
Exab(1)=1, Exab(n)= Exab(n-1)*(2 - 0.1*(ex -2))
Exam(1)=1, Exam(n)= Exam(n-1)*(1.40 - 0.4 *(ex -2))
Exnoa(1)=1, Exnoa(n)= Exnoa(n-1) + (1.0 -
0.1 * (ex - 2))
Exdb(1)=1, Exdb(n)= Exdb(n-1)*(2 - 0.1 * (ex -2)
Exdm(1)=1, Exdm(n)= 0.90^(ex-1)
H: 1.149^(f-1)*(n:1, e:1.5, b:2.0)
Ab: 1.0845^(f-1)*(n:1, e:1.2, b:1.5)
Am: 1.03^(f-1) *(n:1, e:1.02, b:1.05)
Noa: 1.05^(f-1)
Db: 1.0845^(f-1)*(n:1, e:1.2, b:1.5)
Dm: 0.97^(f-1)
```
